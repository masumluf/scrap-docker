...
async function propublicaOld(req, res) {
  ...
  await page.goto(`https://www.propublica.org/topics/${topicName}`, { waitUntil: "networkidle2" });
      ...
      try {
        ...
        const singleData = await newTab.evaluate(async () => {
          try {
            ...
            return {
              author_name: document.querySelector(".byline .name")?.innerText
                ?? "propublica",
              published_at: document.querySelector("time.timestamp")?.dateTime,
        ...
        return [...document.querySelectorAll(".stories .section-articles")].map(
          (element) => {
            return {
              ...
              title: element.querySelector(".hed")?.innerText,
              content_url: element.querySelector(".hed a")?.href,
