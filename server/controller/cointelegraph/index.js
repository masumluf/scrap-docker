...
async function cointelegraph(req, res) {
  let count = 0;
  const browser = await startBrowser();
  page = await startPage(browser);
  const io = req.app.get("io");

  await page.goto("https://cointelegraph.com", { waitUntil: "domcontentloaded" });
      ...
      let results = await page.evaluate(() => {
        return [
          ...document.querySelectorAll(
            ".posts-listing__item > .post-card > article"
          ),
        ].map((element) => {
          const article = {
            domain: "cointelegraph",
            domain_icon_url: document.querySelector("link[rel='shortcut icon']")?.href,
            title: element.querySelector("header")?.innerText,
            content_url: element.querySelector("header a")?.href,
            images_url: element.querySelector("figure img")?.getAttribute("src") ?? "",
            topic: element.querySelector("figure > span")?.innerText.trim(),
            author_name: element.querySelector("footer p")?.innerText.trim() || "cointelegraph",
            published_at: element?.querySelector("footer time")?.getAttribute("datetime"),
            summary: element.querySelector("div p")?.innerText.trim(),
            body: element.querySelector("div p")?.innerText.trim(),
