...
async function foxnews(req, res) {
  let count = 0;
      ...
      try {
        ...
        const singleData = await newTab.evaluate(async () => {
          try {
            ...
            return {
              author_name: document.querySelector(".author-byline a")?.innerText || "foxnews",
  ...
  async function collectData() {
    try {
      ...
      let results = await page.evaluate(() => {
        return [...document.querySelectorAll(".collection-article-list article")].map(
          (element) => {
            const article = {
              domain: "foxnews",
              ...
              topic: element.querySelector(".info-header .eyebrow")?.innerText || "News",
