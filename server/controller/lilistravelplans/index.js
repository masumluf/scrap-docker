...
async function lilistravelplans(req, res) {
  let count = 0;
  const browser = await startBrowser();
  page = await startPage(browser);
  const io = req.app.get("io");

  await page.goto("https://www.lilistravelplans.com/category/stories", { waitUntil: "networkidle2" });
  await scrollPageToBottom(page);
  await page.waitForTimeout(2000);

  async function paginate() {
    try {
      const nextClick = await page.$(".pagination a.prev_button");
      if (nextClick) {
        count++;
        console.log("pagination found");
        console.log(count);
        await Promise.all([page.click(".pagination a.prev_button")]);
        await page.waitForTimeout(2000);
        await scrollPageToBottom(page);
        await page.waitForTimeout(2000);
        await collectData();
    ...
    for (let result of results) {
      ...
      try {
        ...
        const singleData = await newTab.evaluate(async () => {
          try {
            const regexBody = /(<([^>]+)>)|\n|\t|\s{2,}|"|'/ig;
            const body = document.querySelector(".elementor-element-populated")?.innerText.replace(regexBody, " ");
            return {
              images_url: document.querySelector(".elementor-image img")?.src  || "",
  ...
  async function collectData() {
    try {
      ...
      let results = await page.evaluate(() => {
        return [...document.querySelectorAll(".sidebar_content > .type-post")].map(
          (element) => {
            const article = {
              domain: "lilistravelplans",
              ...
              author_name: "lilistravelplans",
              topic: "travel",
              published_at:
                element.querySelector(".post_info_date")?.innerText.replace(/st|nd|rd|th/g, "") ||
                "today",
