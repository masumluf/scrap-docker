...
async function threatpostOld(req, res) {
  ...
  await page.goto(`https://threatpost.com/category/${topicName}`, { waitUntil: "networkidle2" });
  ...
  async function paginate() {
    try {
      const nextClick = await page.$("button#load_more_archive");
      if (nextClick) {
        count++;
        console.log("pagination found...");
        console.log(count);
        await Promise.all([page.click("button#load_more_archive")]);
        await page.waitForTimeout(2000);
        await scrollPageToBottom(page);
        await page.waitForTimeout(2000);
        await collectData();
