...
async function nypostOld(req, res) {
  ...
  await page.goto(`https://nypost.com/${topicName}`, { waitUntil: "networkidle2" });
      ...
      try {
        ...
        const singleData = await newTab.evaluate(async () => {
          try {
            ...
            return {
              topic: document.querySelector(".section-tag")?.textContent.trim() || "News",
              author_name: document.querySelector("#author-byline")?.innerText.trim() || "nypost",
        ...
        return [...document.querySelectorAll(".article-loop .article")].map(
          (element) => {
            const article = {
              ...
              images_url: element.querySelector("picture img")?.currentSrc || "",
              published_at: element.querySelector(".entry-meta p")?.innerText.split("|")[0].trim() || "today",
              summary: element.querySelector(".entry-content")?.innerText.trim(),
