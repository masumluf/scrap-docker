...
async function mylifesamovie(req, res) {
  ...
  await page.goto(`https://mylifesamovie.com/category/${topicName}`, { waitUntil: "networkidle2" });
  await scrollPageToBottom(page);
  await page.waitForTimeout(2000);

  async function paginate() {
    try {
      const nextClick = await page.$(".trawell-pagination a");
      if (nextClick) {
        count++;
        console.log("pagination found");
        console.log(count);
        await Promise.all([page.click(".trawell-pagination a")]);
        await page.waitForTimeout(2000);
        await scrollPageToBottom(page);
        await page.waitForTimeout(2000);
        await collectData();
  ...
  async function collectData() {
    try {
      ...
      let results = await page.evaluate(() => {
        return [...document.querySelectorAll(".container .trawell-posts article")].map(
          (element) => {
            const article = {
              domain: "mylifesamovie",
              domain_icon_url: document.querySelector("link[rel='icon']")?.href,
              title: element.querySelector("h3")?.innerText,
              content_url: element.querySelector("h3 a")?.href,
              author_name: element.querySelector(".meta-author")?.innerText || "mylifesamovie",
              images_url: element.querySelector("img")?.src || "",
              topic: "travel",
              published_at:
                element.querySelector(".meta-date")?.innerText ||
                "today",
