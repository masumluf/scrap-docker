...
async function aljazeeraOld(req, res) {
  let count = 0;
  ...
  await page.goto(`https://www.aljazeera.com/${topicName}`, {
    ...
    for (let result of results) {
      ...
      try {
        ...
        const singleData = await newTab.evaluate(async () => {
          try {
            const body =
              document.querySelector(".gallery-content")?.innerText ??
              document.querySelector(".wysiwyg--all-content")?.innerText;
            return {
              author_name:
                document.querySelector(".article-author-name-item")
                  ?.innerText ?? "aljazeera",
              topic: document.querySelector(".topics")?.textContent ?? "News",
