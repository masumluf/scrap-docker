...
async function devOld(req, res) {
  ...
  await page.goto(`https://dev.to/t/${topicName}`, { waitUntil: "networkidle2" });
  ...
  async function navigateToNextTab(results) {
    for (let result of results) {
      ...
      const singleData = await newTab.evaluate(async () => {
        try {
          const regexBody = /(<([^>]+)>)|\n|\t|\s{2,}|"|'/gi;
          const body = document.querySelector(".crayons-article__body")?.innerText.replace(regexBody, " ");
          return {
        ...
        return [...document.querySelectorAll(".crayons-story__body")].map(
          (element) => {
            const article = {
              ...
              topic:
                element.querySelector(".crayons-tag")?.innerText.replace("#", "") ||
                "coding",
              author_name:
